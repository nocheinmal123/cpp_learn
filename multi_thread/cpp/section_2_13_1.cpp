...
/**
 * 虚假唤醒
 * 往往是wait没有第二个参数导致的，会导致共享数据操作失误
 * 原子操作
 * load 以原子方式读取atomic对象的值
 * store 以原子方式写入atomic对象的值
 * */
class Sample{
    private:
    ...
    public:
        void atomic_test_func() {
            std::atomic<int> mark(flag_.load());
            flag_.store(100);
        }
        // 把收到的玩家命令加到一个队列的线程
        void inMsgRecvQueue() {
        ...
        void outMsgRecvQueue() {
            while (true) {
                ...
                std::cout << "msgRecvQueue front was poped.\n";
...
int main() {
    ...
    std::thread task3(&Sample::inMsgRecvQueue, std::ref(x));
